...
const getTicketNo = () => Math.floor(Math.random() * 900) + 100;
 ...
 */
const roll = async (banner, WishInstance, indexOfBanner, options = {}) => {
	const { forcedRarity } = options;
	...
	const { rarity: rolledRarity } = prob(item);
	const rarity = [3, 4, 5].includes(forcedRarity) ? forcedRarity : rolledRarity;
	let pity = 1;
	...
	const ticketNo = getTicketNo();

	const result = { pity, isNew, bonusType, bonusQty, ticketNo, ...randomItem };
	return result;
};
