<script>
	...
	let sortedWish = [];
	const hideWishExtras = !!localConfig.get('hideWishExtras');
	const sortList = (list) => {
		const item = (star) =>
			list
		...
		<div class="wishlist" bind:clientHeight style="--card-height: {clientHeight}px">
			{#each sortedWish as data, i}
				<div
					class="item-box"
					class:animate={!standalone}
					class:loop={!standalone}
					style="animation-delay: {0.5 + i * 0.1}s; --float-delay: {i * 0.15}s"
					use:removeAnimClass
				>
					<ResultListItem {data} index={i} {clientHeight} {clientWidth} {hideWishExtras} />
				</div>
			{/each}
		</div>
		<div class="shadows" style="--card-height: {clientHeight}px">
			{#each sortedWish as { rarity, type, isNew }, i}
				<div
					...
					use:removeAnimClass
				>
					{#if type === 'character' && !isNew && !hideWishExtras}
...
<style>
	...
	.award-title.animate {
		opacity: 0;
		animation: awardTitleReveal 0.9s ease-out forwards;
	...
	.item-box.loop:not(.animate),
	.shadow.loop:not(.animate) {
		animation: cardFloatLoop 4.5s ease-in-out infinite;
		animation-delay: calc(2.2s + var(--float-delay, 0s));
