<script>
	...
	export let hideWishExtras = false;
	export let data = {};
	const {
		name,
		rarity,
		...
		<div class="zoomist-wrapper item-content" style="--bg: url({$assets['resultcard-bg.webp']})">
			...
			<div class="info" in:fade={{ delay: 1000 }}>
				<div class="icon" style="width: 100%;">
					{#if (isNew || type === 'weapon') && !hideWishExtras}
						{@const iconName = weaponType || vision}
						<SvgIcon name={iconName} />
					{/if}
				</div>

				{#if ((isNew && type === 'character') || type === 'weapon') && !hideWishExtras}
					<div class="star">
				...
				{#if stelaFortuna && !hideWishExtras}
...
<style>
	...
	.ticket-id {
		...
		bottom: -9%;
		transform: translateX(-50%);
		font-size: calc(5.2 / 100 * var(--card-height));
		font-weight: 700;
		line-height: 1;
